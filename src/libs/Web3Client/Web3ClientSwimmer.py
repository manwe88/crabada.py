...
class Web3Client:
    ...
    def signMessage(self, msg) -> SignedMessage:
        """
        Sign the given SignableMessage -> look for SignableMessage class in eth_account.messages.py
        the private key must have been set with setCredential().
        """
        return self.w3.eth.account.sign_message(msg, self.privateKey)
    ...
    def estimateMaxFeePerGasInGwei(self) -> int:
        ...
        baseFeeInGwei = 10000
        if baseFeeInGwei > 10000:
            sys.exit('Too Much Gas Fee')
